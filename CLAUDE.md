...
| F16 | 视觉模型调用点（chat 二轮 sentinel + /explain & /ask 单轮直读） | `llm.py:VisionLLM/OpenAICompatVisionLLM/build_vision_client` + `dispatcher.py:_NEED_IMAGES_RE/_extract_need_images/_resolve_image_paths/_resolve_quoted_image_path/_run_vision_on_quoted_image/chat_assistant 二轮分支/ExplainCommand._execute_vision/AskCommand._execute_vision` + `_CHAT_SYSTEM_PROMPT_VISION_TAIL` + `config.py:vision_*` + `README.md` 配置参考表 + 「视觉模型二轮兜底」段 + `/explain` & `/ask` 段「图片直读」说明 | hook (config.py / dispatcher.py 改触发) | 两类调用路径：(1) chat 自由问答走 sentinel 二轮（模型决定是否要图）；(2) `/explain` & `/ask` 引用图片走单轮直读（用户已指定哪张图，跳过 sentinel）。两类都用同一个 `VisionLLM`。**单轮直读统一走 `_run_vision_on_quoted_image`**——加新 vision 命令时复用这个函数，差异只在 system prompt + user prompt + label 上。**sentinel 标签严格匹配 `<NEED_IMAGES>...</NEED_IMAGES>`**——改 tag 名就要同步 prompt 和正则，否则二轮永不触发。step2 system prompt 用 `_CHAT_SYSTEM_PROMPT_BASE`（不带 tail），且 step2 输出再过一遍 `_extract_need_images` 防泄漏。`f:` 转发子项的 cand_id 静默丢（图未下载）。`WO_VISION_API_KEY` 空 = 全部 vision 功能关闭，chat / explain / ask 都退化成纯文本 |
