...
| F16 | 视觉模型二轮 sentinel 协议 (`<NEED_IMAGES>` + `WO_VISION_*`) | `llm.py:VisionLLM/OpenAICompatVisionLLM/build_vision_client` + `dispatcher.py:_NEED_IMAGES_RE/_extract_need_images/_resolve_image_paths/chat_assistant` 二轮分支 + `_CHAT_SYSTEM_PROMPT` 末段说明 + `config.py:vision_*` + `README.md` 配置参考表 + 「视觉模型二轮兜底」段 | hook (config.py / dispatcher.py 改触发) | sentinel 标签**严格匹配 `<NEED_IMAGES>...</NEED_IMAGES>`**——改 tag 名就要同步 prompt 和正则，否则二轮永不触发。`f:` 转发子项的 cand_id 静默丢（图未下载）。`WO_VISION_API_KEY` 空 = 整个功能关闭，chat 退化成纯文本 |
