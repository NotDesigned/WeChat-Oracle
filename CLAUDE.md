...
| Agent loop / 加 tool / 改记忆表 | `agent/runtime.py` + `agent/tools_read.py` + `agent/tools_write.py` + `agent/persona.py` | F17（必看：group_id 隔离 / 写语义 / msg_id 整数约定） |
...
| F17 | Agent loop 体系（chat 路径 + 四张表 + 工具集 + persona 装配 + dispatcher 集成） | DDL: `schema.sql:persona_drift/member_notes/group_notes/agent_run_log` + 配置: `config.py:agent_*`（base_probability / cooldown_seconds / max_steps / reflect_max_steps / reflection_enabled / personas_dir / recent_context_chat） + 运行时: `agent/runtime.py:run_phase_a/run_phase_b/run_agent` + 工具: `agent/tools.py:Tool/GroupScopedTools/ToolError/StaySilentTool` + Phase A: `agent/tools_read.py:register_phase_a_tools`（recall/view_quoted/expand_forward/read_image/read_voice/who_is/read_member_notes/read_group_notes + stay_silent） + Phase B: `agent/tools_write.py:register_phase_b_tools/phase_b_system_prompt` + 持久化: `agent/memory.py`（DAO） + 路径解析: `agent/media_paths.py` + 人格装配: `agent/persona.py:load_persona_yaml/build_phase_a_system/build_phase_b_system/assemble_system_prompts` + LLM tool-calling: `llm.py:OpenAICompatLLM.complete_with_tools/ToolingLLM/ToolCall/ToolingResponse` + 集成: `dispatcher.py:chat_via_agent/_format_recent_for_agent/_fetch_recent_for_agent/ChatCommand.execute` + 数据: `data/personas/<group_id>.yaml`（用户手编） + 模板: `agent/example_persona.yaml` + 文档: `README.md`「自由问答兜底」段 + 「数据库 schema」agent 四表段 + 配置参考 `WO_AGENT_*` 行 | hook (schema.sql / config.py 改触发) | **关键约束**：`group_id` 不暴露给 LLM 当 tool 参数——所有 SQL 在工具实现里 hardcode `WHERE group_id=?`，参数从 `GroupScopedTools.__init__` 锁定。加新工具必检查这条 invariant。**写语义**：`member_notes` / `persona_drift` 是替换语义（agent 先读再决定全文）；`group_notes` 是追加（事件流）；`agent_run_log` 始终追加（审计）。改语义 = 数据契约破坏。**msg_id 格式**：agent 工具一律收 `int`，不是 `/find` 的 `m:N` / `f:N` 字符串——别把两套混了。**触发层**：今天 `chat_via_agent` 只在 `ChatCommand.execute` 里被调，`trigger_kind` 永远是 `'mention'`。要加 reply / probability 触发需要在 `_next_unprocessed` 上游加钩子（暂未做）。**Phase B 运作方式**：`reflection_enabled=False` 时直接跳过；启用时模型可以选择 0 写入（输出空文本结束）。新加 write 工具时记得在 `register_phase_b_tools` 注册；Phase B 也要保留对应的 read 工具方便先读再写 |
